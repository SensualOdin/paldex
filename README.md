...
- **Team synergy planner** (Team tab): build a party of 5 and the planner parses partner skills + guaranteed passives into effect tags (attack-type imbues, element damage boosts, team Attack buffs, resistances) and surfaces cross-Pal synergies — e.g. an imbuer that turns your attacks Ground paired with a Ground-damage booster — plus offensive type coverage, uncovered elements, and shared weaknesses. Persisted in localStorage. Also includes:
  - **Passive & effect search** — type "poison", "lifesteal", "vanguard"… and get every matching passive (from the full 115-passive game table, with exact effects, rank and how to get it) plus every Pal whose partner skill or guaranteed passives match, with one-tap add-to-team.
  - **Best combo per element** — for each of the 9 elements, an auto-computed **Player damage build** (imbuer → converts your attacks, damage boosters, top attackers, Vanguard tip) and **Pal damage build** (top attackers + party Attack auras + a suggested 4-passive surgery kit), each with a one-click "Load this team".
  - **Pal Surgery planner** per slot: pick up to 4 passives (guaranteed ones pre-filled; full searchable picker with surgery gold costs vs inherit-only) and see the **projected Attack, element damage and stat tier** (Attack × √(HP × Defense) percentile — S/A/B/C/D) update live.
...
- **Interactive world map** (Map tab): canvas pan/zoom over the game's own map tiles with **13,800+ markers** from the game's tables — fast travel, Alpha Pals (with level), dungeons, towers, effigies, eggs, chests, every ore/coal/quartz/sulfur/soralite node, fishing spots, merchants — across **two maps** (Palpagos Islands at 4096px + The World Tree). Markers cluster into category-colored count bubbles at low zoom, get pin badges + name labels up close, and popups show Alpha Pal mini-stats, the **nearest fast travel statue with distance/direction**, and a **shareable location link** (`#map=main/x/y`). Category filters with presets (Essentials / Farming run / Collector), marker search + coordinate jump ("337,360"), in-game coordinate readout, **unlimited free found-tracking** with per-type progress and hide-found, **custom pins with notes** (double-click), progress **export/import** (no account needed), up to **three color-coded Pal spawn layers** overlaid at once with a day/night toggle, plus **click any empty spot to see every Pal that spawns there** (day and night, straight from the game's distribution table), and popups that deep-link into the dex (Alpha Pals) and item pages (resource nodes). Plus **calculated farming routes**: pick Bounties, Holy Water & Relics (World Tree Awakened + Alpha spawns), World Tree Eggs, Tree Effigies or Palpagos Alphas and the map draws the **shortest round trip** — numbered stops, direction arrows, back to the start (nearest-neighbor + 2-opt; the 66-bounty loop optimizes to ~41 km from ~283 km in data order). Stop 1 starts next to a fast travel statue, marking a stop as found re-optimizes the loop without it, and one tap reverses direction. All offline, zero ads.
...
python _fetch_passives.py        # full passive table: effects, ranks, surgery costs -> _passives.json
